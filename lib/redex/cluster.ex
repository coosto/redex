defmodule Redex.Cluster.Strategy.Kubernetes do
  @moduledoc """
  ...
  defp get_nodes(%State{topology: topology, config: config}) do
    ...
    cond do
      app_name != nil and selector != nil ->
        ...
        endpoints_path = "api/v1/namespaces/#{namespace}/pods?#{query}"
  ...
  defp parse_response(:ip, app_name, resp) do
    case resp do
      %{"items" => items} when is_list(items) ->
        Enum.map(items, fn item -> :"#{app_name}@#{item["status"]["podIP"]}" end)
    ...
    case resp do
      %{"items" => items} when is_list(items) ->
        Enum.map(items, fn item ->
          format_dns_record(app_name, item["status"]["podIP"], item["metadata"]["namespace"])
        end)
