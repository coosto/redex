defmodule Redex.Command.RpushTest do
  use ExUnit.Case, async: true
  use ExUnitProperties

  import Mox
  import Redex.DataGenerators
  import Redex.Command.RPUSH

  setup :verify_on_exit!

  property "RPUSH a non existing or expired key" do
    check all state = %{db: db} <- state(),
      ...
      |> expect(:write, fn :redex, {:redex, {^db, ^key}, ^args, nil}, :write -> :ok end)
  ...
  property "RPUSH an existing key" do
  ...
  property "RPUSH a key with a wrong kind of value" do
  ...
  property "RPUSH in readonly mode" do
  ...
  property "RPUSH with wrong number of arguments" do
    error = {:error, "ERR wrong number of arguments for 'RPUSH' command"}
